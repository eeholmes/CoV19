...
```{r}
reg <- "Lombardia"
p <- ggplot(subset(datL, region==reg & lag%in%c(0,3,5,7,10) & date < as.Date("2020-06-01")), aes(x=x,y=log.val,color=lag)) + geom_line() + geom_point() +
...
reg <- "Italy"
p1b <- plotcumfun(dat, reg, c(-6,-1), lags=5)
...
reg <- "United Kingdom"
p1 <- plotfun(dat, reg, c(-4,0))
p1b <- plotcumfun(dat, reg, c(-6,-1), lags=7)
gridExtra::grid.arrange(p1,p1b, nrow=1)
```
...
p1b <- plotcumfun(dat, reg, c(-6,-1), lags=c(10,7))
gridExtra::grid.arrange(p1,p1b, nrow=1)
```
