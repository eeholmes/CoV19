...
```{r}
dat <- c()
for(reg in c("Italy","France","Canada", "US","Spain","United Kingdom","Austria","Germany", "Switzerland", "Greece", "Portugal", "Sweden", "Denmark", "Belgium", "Netherlands", "Iran", "China")){
...
In Italy, the flat line is 3-day lag, which would suggest that people were not coming in to get tested until they were very sick and close to dying.
...
Germany is a big exception. We can see a clear effect of early testing, well before people are very sick (and close to dying).

```{r}
reg <- "Germany"
plotfun(dat, reg, c(-6,-2))
```

Iran is odd. Lines should not go down like that unless a) early death rate was much higher or b) they started under-reporting deaths at some point.
...
The flattening lags for the US is about the same, 5 to 7 days prior. But look at the lagged CFR (lag 5 to 7). It is much lower than in Europe.  Lag is 5-7 days, suggesting that people are a) getting tested when they are less sick, b) they live longer once hospitalized, or c) there is a longer lag in reporting deaths than in Europe.

```{r}
reg <- "US"
plotfun(dat, reg, c(-5,-1))
```
...
```{r}
datS <- c()
for(reg in c("NY","NJ","WA","CA","LA","MI")){
...
```{r}
reg <- "NY"
plotfun(datS, reg, c(-5,-1), lags=c(0,3,5,7,10,15))
```

Michigan is weird. No idea why the lag would be so long (2 weeks).
...
Louisiana is weird. It reminds me of Iran with higher death rate early and then declining.

```{r}
reg <- "LA"
plotfun(datS, reg, c(-4,0), xlims=c(100,1000))
```
