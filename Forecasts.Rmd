...
if("DC" %in% reg) regfull <- c("DC", regfull)
popmill <- round(sum(statepop$POPESTIMATE2019[match(regfull, statepop$NAME)], na.rm=TRUE)/1e6, digits=2)
...
if(inherits(p, "list")){
popmill <- 0.6*statepop$POPESTIMATE2019[match(c("California"), statepop$NAME)]/1e6
    p$p <- p$p + scale_y_continuous(sec.axis = sec_axis(~ ./popmill, name="new cases/million"))
  plot(p$p)
}
```
