...
fitf1<- function(subx, m=20, a=1, s=0.08, xshift=0){
  f1<-function(x, theta, a) {
    m<-theta[1]; s<-theta[2];
...
getcis <- function(fit.best, subx, m=20, s=0.2, sig=0.05, level=0.05, xshift=0){
  ...
  for(a in aval){
    ...
    if(!is.na(tauval) && tauval>3 && tauval<5) next #catch bad fits with < 5
...
```{r myfun}
myfun <- function(data, reg, posmin=100, maxh=100, xshift=0, bad.reg=NULL, fun="f2", s.fix=0.08, sig=0.05){
...
for(i in c("NY", "NJ", "MA", "CT", "DE", "PA", "MD", "ME", "NH", "DC")){
...
for(i in c("Washington US", "California US", "Oregon US")){
  p <- myfun(world, i, posmin=100, maxh=150)
  if(inherits(p, "list")) plot(p$p)
}
```
