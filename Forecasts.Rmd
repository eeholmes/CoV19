...
If the model fits, then the blue dots are the smoothed (3-day average) new cases to which the model was fit.
...
getfit <- function(x, posmin=100, maxh=200, xshift=0, fun="f2", s.fix=0.08, sig=0.05){
  ...
  subx$y <- subx$y/maxy
  for(s in c(0.05,.1,1,seq(0.2,3,0.5))){
    for(m in seq(10,200,10)+xshift){
...
```{r myfun}
myfun <- function(data, reg, regname="", posmin=100, maxh=200, mindate="2020-02-15", maxdate="2020-09-01", xshift=0, bad.reg=NULL, fun="f2", s.fix=0.08, sig=0.05){
  ...
  if(missing(regname)) reg <- paste(reg, collapse="+") else reg <- regname
  b$region <- reg
...
for(i in c("Denmark", "Finland", "Norway",
           "Sweden")){
...
```{r  CEurope-forecast}
for(i in c("Austria", "Czechia", "Hungary", "Poland",
           "Slovenia")){
  p <- myfun(world, i, posmin=100, maxh=150)
  if(inherits(p, "list")) plot(p$p)
}
...
```{r  SEurope-forecast}
for(i in c("France", "Italy", "Portugal",
           "Spain")){
  p <- myfun(world, i, posmin=100, maxh=150)
  if(inherits(p, "list")) plot(p$p)
}
...
p <- myfun(states, c("ME","VT","NH","MA","RI","CT"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
p <- myfun(states, c("NY","NJ","PA"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
p <- myfun(states, c("OH", "IN", "IL", "WI", "MI"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
p <- myfun(states, c("MN", "MO", "ND", "SD", "IA", "NE", "KS"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
```{r states-forecast-southatlantic}
p <- myfun(states, c("DC", "DE", "VA", "WV", "MD"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
p <- myfun(states, c("GA", "FL", "SC", "NC"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
p <- myfun(states, c("AL", "MS", "KY", "TN"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
p <- myfun(states, c("TX", "OK", "AR"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
p <- myfun(states, c("ID", "MT", "WY"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
p <- myfun(states, c("CO", "UT", "NV", "CO", "NM", "AZ"), posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
reg <- c("San Diego California US",
         ...
         "Santa Barbara California US",
         "San Luis Obispo California US")
p <- myfun(world, reg, regname="Southern CA", posmin=100, maxh=150)
if(inherits(p, "list")) plot(p$p)
```
...
for(i in c("NY", "NJ", "MA", "CT", "DE", "PA", "MD", "DC")){
  p <- myfun(states, i, posmin=10, maxh=150)
  if(inherits(p, "list")) plot(p$p)
}
```
...
for(i in c("WI", "MN", "SD", "ND", "IA", "KS", "NE")){
  p <- myfun(states, i, posmin=10, maxh=150)
  if(inherits(p, "list")) plot(p$p)
}
```
...
```{r states-forecast-westcoast, eval=FALSE}
for(i in c("WA", "CA", "OR")){
  p <- myfun(states, i, posmin=100, maxh=150)
  if(inherits(p, "list")) plot(p$p)
}
...
```{r states-forecast-westcoast2}
for(i in c("Washington US", "Oregon US")){
  p <- myfun(world, i, posmin=100, maxh=150)
  if(inherits(p, "list")) plot(p$p)
}
```
