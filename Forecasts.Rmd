...
for(i in c("NY", "NJ", "MA", "CT", "DE", "PA", "MD", "MI", "OH", "IL", "IN", "WA", "CA", "OR", "CO", "LA")){
  p1 <- myfun(states, i, posmin=10, maxh=150, fun="f4", s.fix=s)
  p2 <- myfun(states, i, posmin=10, maxh=150)
  b<- theme(plot.title = element_text(size = 6))
  if(inherits(p1, "list") & inherits(p2, "list")) grid.arrange(p1$p+b, p2$p+b, nrow=1)
}
```
