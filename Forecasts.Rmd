...
myfun <- function(data, reg, regname="", posmin=100, maxh=200, mindate="2020-02-15", maxdate="2020-09-01", xshift=0, bad.reg=NULL, fun="f2", s.fix=0.08, sig=0.05){
  ...
  if(inherits(fit$fit, "try-error")){
    ...
    popmill <- round(sum(popdata$population[match(c(reg, regname), popdata$name)], na.rm=TRUE)/1e6)
...
```{r  CEurope-forecast}
for(i in c("Czechia", "Hungary", "Poland",
           "Slovenia")){
  worldplot(i)
}
```
...
```{r stateplotfun}
stateplot <- function(reg, posmin=100, maxh=150){
#regfull <- state.name[match(reg, state.abb)]
popmill <- round(sum(popdata$population[match(reg, popdata$name)], na.rm=TRUE)/1e6, digits=2)
p <- myfun(states, reg, posmin=posmin, maxh=maxh)
if(inherits(p, "list")){
  p$p <- p$p+annotate("text", x=as.Date("2020-09-01"), y=Inf, label=paste("population =", popmill, "million"), hjust=1, vjust=2)
  #p$p <- p$p+ylim(0, max(500*popmill,layer_scales(p$p)$y$range$range[2]))
  p$p <- p$p + scale_y_continuous(sec.axis = sec_axis(~ ./popmill, name="new cases/million"))
 plot(p$p)
}
}
...
```{r states-forecast-mtnsw-nonv}
stateplot(c("CO", "UT", "NM", "AZ"))
```
...
```{r states-forecast-lacounty}
reg <- "Los Angeles California US"
p <- myfun(world, reg, regname="LA County CA", posmin=100, maxh=150)
if(inherits(p, "list")){
popmill <- 10.04
    p$p <- p$p + scale_y_continuous(sec.axis = sec_axis(~ ./popmill, name="new cases/million"))
  p$p <- p$p+annotate("text", x=as.Date("2020-09-01"), y=Inf, label=paste("population =", popmill, "million"), hjust=1, vjust=2)
...
for(i in c("WI", "MN", "SD", "ND", "IA", "NE", "MO")){
  stateplot(i, posmin=100)
}
```
