...
for(a in aval){
  ...
  if(tauval>3 & tauval<5) break #catch bad fits with < 5
...
cm1=NA; cm2=NA
confval <- try(confint(fit, level=0.95), silent=TRUE)
if(inherits(confval, "try-error") || summary(fit)$sigma<0.03 || any(is.na(confval)) ) confval <- try(getcis(fit.best=fit, subx=subx, level=0.05, xshift=xshift, sig=sig))
...
p <- ggplot(b, aes(x = date, y = new.cases.smooth)) + geom_point() + 
  ...
  bm <- 1.05*max(b$new.cases, pred$pred, pred$pred.high, na.rm=TRUE)
...
for(i in c("Brazil", "Chile")){
  p <- myfun(world, i, posmin=10, maxh=150)
  if(inherits(p, "list")) plot(p$p)
}
```
