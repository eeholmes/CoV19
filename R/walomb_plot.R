my_plot <- function(x, reg, lockdown=NULL){
  ...
  p <- p + geom_vline(xintercept = as.Date(lockdown)) +
    annotate("text", x=as.Date(lockdown)+1, y=.5*ymax, hjust=0, label=paste(reg, "lockdown"), color="red", size=3, angle=90)
  p <- p + geom_vline(xintercept = as.Date(lockdown)+12, linetype = "dashed") +
    annotate("text", x=as.Date(lockdown)+12+1, y=.5*ymax, hjust=0, label="12 days past lockdown", color="red", size=3, angle=90)
  }
  p
}
