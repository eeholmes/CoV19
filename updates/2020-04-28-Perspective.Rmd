---
title: "Epidemic impacts in different regions"
date: April 28, 2020
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.height=6)
```

```{r child="Forecast-Functions.Rmd"}
```
...
```{r}
library(ggplot2)
df <- data.frame(name="Mid-Atlantic", peak.cpm=350, curr.dpt=0.6, proj.dpt=1.18, cfr=7)
df <- rbind(df, data.frame(name="New England", peak.cpm=300, curr.dpt=0.35, proj.dpt=NA, cfr=8))
df <- rbind(df, data.frame(name="NY", peak.cpm=550, curr.dpt=0.44, proj.dpt=0.77, cfr=7))
df <- rbind(df, data.frame(name="NJ", peak.cpm=400, curr.dpt=0.33, proj.dpt=1, cfr=7))
df <- rbind(df, data.frame(name="MA", peak.cpm=400, curr.dpt=0.21, proj.dpt=NA, cfr=8.2))
df <- rbind(df, data.frame(name="NJ", peak.cpm=400, curr.dpt=0.33, proj.dpt=1, cfr=7))
df <- rbind(df, data.frame(name="SE Atlantic", peak.cpm=60, curr.dpt=0.06, proj.dpt=0.16, cfr=4.6))
df <- rbind(df, data.frame(name="FL", peak.cpm=50, curr.dpt=0.03, proj.dpt=0.05, cfr=4.2))
df <- rbind(df, data.frame(name="TX+OK+AR", peak.cpm=30, curr.dpt=0.02, proj.dpt=0.12, cfr=3.9))
df <- rbind(df, data.frame(name="Mtn North: ID+MT+WY", peak.cpm=40, curr.dpt=0.03, proj.dpt=0.03, cfr=3.2))
df <- rbind(df, data.frame(name="SoCal", peak.cpm=75, curr.dpt=0.05, proj.dpt=NA, cfr=NA))
df <- rbind(df, data.frame(name="NorCal", peak.cpm=20, curr.dpt=0.03, proj.dpt=0.06, cfr=4.4))
df <- rbind(df, data.frame(name="WA", peak.cpm=50, curr.dpt=0.03, proj.dpt=0.05, cfr=6))
p<-ggplot(data=df, aes(x=name, y=peak.cpm)) +
  geom_bar(stat="identity")
p + coord_flip() + xlab("Peak cases per million")
```
...
library(gridExtra)
library(ggplot2)
substates <- subset(states, date< as.Date("2020-04-28"))
ps <- list()
ps[["MidAtlantic"]] <- stateplot(c("NY","NJ","PA"), data=substates, sec.axis.name="") + ggtitle("NY+NJ+PA") + ylab("") + xlab("")
val <- list("MA", c("MD","DE"), c("CT","RI"),"VA","NC")
for(i in 1:length(val)) 
  ps[[paste0(val[[i]], collapse="+")]] <- stateplot(reg=val[[i]], data=substates, fitit=FALSE, sec.axis.name="")+ggtitle(paste0(val[[i]], collapse="+")) + ylab("") + xlab("")
for(i in c("GA","FL")) 
  ps[[i]] <- stateplot(reg=i, data=substates, sec.axis.name="")+ggtitle(i) + ylab("") + xlab("")
grid.arrange(grobs=ps, ncol=2,
           left = textGrob("7-day average new cases per day", rot = 90, vjust = 1),
           right = textGrob("7-day average new cases per day per million", rot = 90, vjust = 0))
```
...
substates <- subset(states, date< as.Date("2020-04-28"))
ps <- list()
ps[["MidWest1"]] <- stateplot(c("OH", "IN", "IL", "WI", "MI"), data=substates, fitit=FALSE, sec.axis.name="") + ggtitle("East North Central Midwest") + ylab("") + xlab("")
ps[["MidWest2"]] <- stateplot(c("MN", "MO", "ND", "SD", "IA", "NE", "KS"), data=substates, fitit=FALSE, sec.axis.name="") + ggtitle("West North Central Midwest") + ylab("") + xlab("")
ps[["IANE"]] <- stateplot(c("IA", "NE"), data=substates, fitit=FALSE, sec.axis.name="") + ggtitle("IA+NE") + ylab("") + xlab("")
ps[["FourCorners"]] <- stateplot(c("CO", "UT", "NM", "AZ"), data=substates, fitit=FALSE, sec.axis.name="") + ggtitle("CO+UT+NM+AZ") + ylab("") + xlab("")
ps[["East South Central"]] <- stateplot(c("AL", "KY", "TN", "MS"), data=substates, fitit=FALSE, sec.axis.name="") + ggtitle("East South Central") + ylab("") + xlab("")

subworld <- subset(world, date< as.Date("2020-04-28"))
...
p <- myfun(subworld, c("Washington US", "Oregon US", ncacounties), regname="West minus SoCal", posmin=100, maxh=150)
...
popmill <- round(popmill/1e6, digits=2)
  ...
  p$p <- p$p+ scale_y_continuous(sec.axis = sec_axis(~ ./popmill, name=""))
ps[["West"]] <- p$p + ylab("") + xlab("") + ggtitle("WA+OR+NorCal")
reg <- c("San Diego California US",
         "San Bernardino California US",
         "Riverside California US",
         "Orange California US",
         ...
         "Santa Barbara California US",
         "San Luis Obispo California US")
p <- myfun(subworld, reg, regname="Southern CA", posmin=10, maxh=150)
if(inherits(p, "list")){
popmill <- round(0.6*statepop$POPESTIMATE2019[match(c("California"), statepop$NAME)]/1e6, digits=2)
    p$p <- p$p + scale_y_continuous(sec.axis = sec_axis(~ ./popmill, name=""))
  p$p <- p$p+annotate("text", x=as.Date("2020-09-01"), y=Inf, label=paste("population =", popmill, "million"), hjust=1, vjust=2)
ps[["SoCal"]] <- p$p + ylab("") + xlab("") + ggtitle("SoCal")
}
