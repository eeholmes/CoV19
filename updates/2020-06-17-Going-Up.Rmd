...
Right now in Arizona, the Carolinas, and Alabama, the per capita new cases is getting into the +150 per million zone where health care system are hard hit and control of the epidemic is lost (meaning community spread is so pervasive that mitigations don't stop a rapid rise in cases). This plot shows the states (in red) where the current new cases per day are above 100 and cases are going up fast. For comparison, the peak new cases per day in three hard hit east coast states are shown in blue.
...
ggplot(data=subb, aes(x=region, y=max.smooth/popmill, fill=fill)) +
  ...
  ggtitle("Current new cases per day for AZ, NC, SC and AL (with NY, NJ, MA April maxes for comparison)")
```

```{r states-forecast-going-up}
grid.arrange(stateplot("AZ", fitit=FALSE), 
             stateplot("NC", fitit=FALSE),
